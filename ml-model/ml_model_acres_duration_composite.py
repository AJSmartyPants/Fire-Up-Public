...
# Filter valid targets & build X / y
...
# === SINGLE SPLIT (same indices for both targets) ===
idx = np.arange(len(X))
...
# === STRONGER, SANE DEFAULTS (no grid search) ===
models = {
    # Linear baselines
...
def leaderboard(X_tr, y_tr, X_te, y_te, label):
    ...
    for name, model in models.items():
        # log1p target transform (helps heavy-tailed targets)
...
# === EXPORT BEST LIGHTGBM MODEL TRAINED ON FULL DATA ===
...
print(" - ml-model/lightgbm_duration.joblib")
print(" - ml-model/lightgbm_acres.joblib")
